...
export default function VideoCard({ thumbnail, title, href, cta = "Watch now" }) {
  return (
    <Link
      href={href}
      className="rounded-2xl bg-beige shadow-[0_2px_8px_rgba(0,0,0,0.06)] flex flex-col group transition-shadow duration-200 hover:bg-emerald-700/30 hover:shadow-[0_4px_16px_0_rgba(0,0,0,0.10)] focus:shadow-[0_4px_16px_0_rgba(0,0,0,0.10)] outline-none overflow-hidden"
      target="_blank"
    ...
    >
      ...
      <div className="p-10 flex flex-col flex-1 transition-transform duration-200 group-hover:scale-105 will-change-transform origin-center">
        ...
        <span className="mt-auto flex items-center gap-1 font-semibold text-sm group-hover:underline group-focus:underline">
          {cta}
